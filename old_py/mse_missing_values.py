...
def missing_mse(y_true, y_pred):
    ...
    >>> float_missing_test = K.cast(tf.logical_not(tf.equal(y_true_test,-1.0)), dtype='float32')
    >>> tf_float_missing = sess.run(float_missing_test)
    >>> tf_float_missing
    array([[0., 1.],
           [1., 0.]], dtype=float32)
    >>> loss_test = K.mean(K.square( (y_pred_test - y_true_test)*tf_float_missing ), axis=-1)
    >>> sess.run(loss_test)
...
def missing_mse2(y_true, y_pred):
    ...
    >>> from keras import backend as K
    >>> sess = tf.Session()
    >>> y_true_test = tf.constant([[-1.0,2.0],[5.0,-1.0],[-1.0,-1.0]])
    >>> y_pred_test = tf.constant([[2.0,4.0],[3.0,2.0],[5.0,14.0]])
    ...
    array([[False,  True],
           [ True, False],
           [False, False]])
    >>> tf_float_missing = K.cast(bool_missing, dtype='float32')
    >>> sess.run(tf_float_missing)
    array([[0., 1.],
           [1., 0.],
           [0., 0.]], dtype=float32)
    >>> missing_squared = K.square( (y_pred_test - y_true_test)*tf_float_missing )
    ...
    >>> loss_scalar = K.mean(masked, axis=-1)
    >>> sess.run(loss_scalar)
    4.0
    >>> assert (sess.run(missing_mse(y_true_test, y_pred_test)) == sess.run(loss_scalar))
    """
